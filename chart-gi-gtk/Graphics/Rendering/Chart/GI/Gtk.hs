...
createRenderableWindow chart windowWidth windowHeight = do
    ...
    Gtk.onWidgetDraw canvas $ renderWithContext (drawCanvasHandler canvas chart)

    return window

drawCanvasHandler :: Gtk.DrawingArea -> Renderable a -> C.Render Bool
drawCanvasHandler canvas chart = do
  mwin <- Gtk.widgetGetWindow canvas
  case mwin of
    Nothing -> return False
    Just win -> do
      width  <- C.liftIO $ Gtk.widgetGetAllocatedWidth canvas
      height <- C.liftIO $ Gtk.widgetGetAllocatedHeight canvas
